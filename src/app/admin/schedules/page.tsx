...
export default function AdminSchedules() {
  ...
  const [form, setForm] = useState({ doctorId: '', dayOfWeek: '', startTime: '', endTime: '', availabilitySlots: '10' });
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    await fetch('/api/schedules', {
      ...
      body: JSON.stringify({ ...form, doctorId: parseInt(form.doctorId), availabilitySlots: parseInt(form.availabilitySlots) }),
    });
    setLoading(false);
    setShowModal(false);
    setForm({ doctorId: '', dayOfWeek: '', startTime: '', endTime: '', availabilitySlots: '10' });
    fetchSchedules();
  }
  ...
  return (
    <>
      <div className="dash-topbar">
        <h1 className="dash-title">Schedules</h1>
      ...
      <div className="dash-body">
        <div className="card">
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Home</th><th>Doctor</th><th>Specialization</th><th>Day</th><th>Start</th><th>End</th><th>Slots</th><th>Status</th></tr>
              </thead>
              <tbody>
                {schedules.map(s => (
                  <tr key={s.scheduleId}>
                    <td><Link href="/?home=1" className="link">🏠</Link></td>
                    <td style={{ fontWeight: 500 }}>{s.doctor?.fullName}</td>
                ...
                {!schedules.length && (
                  <tr><td colSpan={8} style={{ textAlign: 'center', color: 'var(--gray2)', padding: '2rem' }}>No schedules yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? 'Adding...' : 'Add Schedule'}</button>
              </div>
            </form>
          </div>
        </div>
