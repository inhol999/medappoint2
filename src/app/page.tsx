...
export default function HomePage() {
  ...
  // Show dashboard only if logged in AND not requesting home page
  if (session && !showHome) {
  ...
  // Show landing page (for logged-out users OR logged-in users who clicked Home)
  return (
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-card { background: white; border: 1px solid #e2e8f0; border-radius: 12px; padding: 2rem; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
        .stat-value { font-size: 2.5rem; font-weight: 600; color: #1e293b; margin-bottom: 0.5rem; }
      ...
      <nav className={scrolled ? 'scrolled' : ''}>
        ...
        <div className="nav-right">
          {session ? (
            <Link href="/dashboard" className="nav-signin" style={{ fontWeight: '500', color: '#1e293b' }}>
              👤 {(session.user as any)?.name || 'Dashboard'}
            </Link>
          ) : (
            <>
      ...
      <section className="features-section">
        <div className="features-inner">
          ...
          <div className="features-grid">
            {[
              { icon: '🔍', title: 'Smart Search', desc: 'Find clinics by specialty, location, or doctor name with advanced filters.' },
              { icon: '📅', title: 'Instant Booking', desc: 'Book appointments 24/7 with real-time availability and instant confirmation.' },
              { icon: '💬', title: 'Direct Communication', desc: 'Message your doctor directly through our secure platform.' },
              { icon: '📊', title: 'Health Records', desc: 'Access your medical history, prescriptions, and test results anytime.' },
              { icon: '⭐', title: 'Rated & Reviewed', desc: 'Read reviews from real patients to choose the best healthcare providers.' },
              { icon: '💳', title: 'Secure Payments', desc: 'Pay for appointments safely with multiple payment options.' },
            ].map(f => (
          ...
          {tab === 'patient' && (
            <div className="who-body">
              ...
              <div className="who-cta-wrap">
                ...
                <p>No more calling clinic after clinic. Find the right doctor, at the right clinic, at a time that works.</p>
                {!session && <Link href="/register" className="btn-dark">Create patient account</Link>}
                {session && <Link href="/dashboard" className="btn-dark">Go to Dashboard</Link>}
              </div>
            </div>
          )}
          {tab === 'clinic' && (
            <div className="who-body">
              <div className="who-points">
                {[
                  { title: 'Register your clinic', desc: 'List your clinic on MedaPoint and reach thousands of patients.' },
              ...
              <div className="who-cta-wrap">
                ...
                <p>Digitize operations, reduce admin work, and deliver a better patient experience.</p>
                {!session && <Link href="/login" className="btn-dark">Sign in as Admin</Link>}
                {session && <Link href="/dashboard" className="btn-dark">Go to Dashboard</Link>}
              </div>
            </div>
          )}
          {tab === 'doctor' && (
            <div className="who-body">
              ...
              <div className="who-cta-wrap">
                ...
                <p>A clean, fast dashboard that puts everything you need in one place.</p>
                {!session && <Link href="/login" className="btn-dark">Go to doctor portal</Link>}
                {session && <Link href="/dashboard" className="btn-dark">Go to Dashboard</Link>}
              </div>
            </div>
          )}
